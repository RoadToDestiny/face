...
В `scripts/run_ui.py` для загруженного видео теперь используется офлайн-предобработка перед стартом воспроизведения:
- извлекается аудиодорожка (через `ffmpeg`, видео должно быть со звуком)
- VOSK строит субтитры с таймкодами
- RuBERT определяет эмоцию текста (папка модели ищется автоматически)
- по кадрам заранее вычисляется эмоция лица

После завершения предобработки запускается проигрывание: на экране показываются предрассчитанные эмоции и цветные субтитры (цвет зависит от эмоции текста).

Если RuBERT недоступен (нет зависимостей или модель не загрузилась), видео всё равно запускается: субтитры остаются, а эмоция текста выставляется как `NEUTRAL`.

Для более точного кадрирования лица уменьшен дополнительный отступ при вырезке лица (crop padding), поэтому кроп меньше захватывает фон вокруг лица.
